  ...
  const stopApp = useCallback(async () => {
    ...
    if (currentUid && sessionId && startedAt) {
      ...
      const finalized = await finalizeSessionSummary(currentUid, sessionId, {
        ...
        bestImageAnalysis: posture.bestSnapshotRef.current?.imageAnalysis ?? null,
        bestImageLandmarks: posture.bestSnapshotRef.current?.imageLandmarks ?? null,
        ...
        worstImageAnalysis: posture.worstSnapshotRef.current?.imageAnalysis ?? null,
        worstImageLandmarks: posture.worstSnapshotRef.current?.imageLandmarks ?? null,
